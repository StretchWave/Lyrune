...
from logger import log_event, log_once
...
class MediaWorkerThread(QThread):
    ...
    def run(self):
        ...
        if sys.platform == "win32":
            try:
                ctypes.windll.ole32.CoInitializeEx(None, 2)  # COINIT_APARTMENTTHREADED for WinRT async dispatch
...
class SpotifyPlayer:
    ...
    def _fetch_window_title_fallback(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if found_title:
                ...
                if c_tit and c_tit.lower() not in ['spotify', 'spotify free', 'spotify premium', 'web player']:
    ...
    async def _fetch_winrt_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Scan all sessions — prioritize Playing sessions first
            best_playing_session = None
            best_fallback_session = None
                ...
                for i, s in enumerate(sessions):
                    ...
                    if c_tit:
                        ...
                        if self._target_source != "Auto-Detect":
                            ...
                            # Auto-Detect: prefer Playing status (4), fallback to any valid title
                            if status_num == 4 and not best_playing_session:
                                best_playing_session = (s, p, c_art, c_tit)

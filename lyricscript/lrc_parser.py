...
class LRCParser:
    ...
    # Matches [offset: +/-ms] metadata tag
    _OFFSET_RE = re.compile(r'\[offset:\s*([+-]?\d+)\]', re.IGNORECASE)
    ...
    def __init__(self, lrc_text: Optional[str] = None):
        ...
        self.global_offset_ms: int = 0
        if lrc_text:
            self.parse(lrc_text)

    def parse(self, lrc_text: str) -> None:
        """Parses LRC text into sorted timestamp-lyric pairs, honoring [offset: +/-ms]."""
        pairs: List[Tuple[float, str]] = []
        self.global_offset_ms = 0
        ...
        # First pass: check for global [offset:] header tag
        for raw_line in lrc_text.splitlines():
            line = raw_line.strip()
            off_match = self._OFFSET_RE.search(line)
            if off_match:
                try:
                    self.global_offset_ms = int(off_match.group(1))
                except ValueError:
                    pass

        offset_sec = self.global_offset_ms / 1000.0
        ...
        for raw_line in lrc_text.splitlines():
            ...
            for hours_str, mins_str, secs_str, ms_str in timestamps:
                ...
                total_seconds = hours * 3600 + minutes * 60 + seconds + fraction + offset_sec
                pairs.append((max(0.0, total_seconds), lyric_text))
